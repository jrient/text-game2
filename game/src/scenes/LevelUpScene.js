...
export default class LevelUpScene extends Phaser.Scene {
  ...
  create() {
    ...
    sep.lineStyle(2, 0x3355aa); sep.lineBetween(30, H * 0.23, W - 30, H * 0.23);
  ...
  _buildCard(cx, cy, choice, idx) {
    ...
    const borderCol = skill.type === 'weapon' ? 0x4466cc : 0x449944;
    bg.fillStyle(0x111133, 0.95); bg.fillRoundedRect(-W / 2, -H / 2, W, H, 6);
    bg.lineStyle(3, borderCol);   bg.strokeRoundedRect(-W / 2, -H / 2, W, H, 6);
    container.add(bg);

    // Shine strip
    const shine = this.add.graphics();
    shine.fillStyle(0xffffff, 0.06);
    shine.fillRect(-W / 2 + 4, -H / 2 + 4, W - 8, 10);
    container.add(shine);
    ...
    if (isNew) {
      const badge = this.add.text(W / 2 - 8, -H / 2 + 8, 'NEW', {
        fontFamily: "'Press Start 2P'", fontSize: '7px', color: '#ffff44',
        backgroundColor: '#aa2200', padding: { x: 4, y: 2 },
      }).setOrigin(1, 0);
      container.add(badge);
    } else {
      const lvBadge = this.add.text(W / 2 - 8, -H / 2 + 8, `Lv.${nextLevel}`, {
        fontFamily: "'Press Start 2P'", fontSize: '7px', color: '#44ffaa',
    ...
    // Skill name
    const nameColor = skill.type === 'weapon' ? '#88aaff' : '#88ff88';
    ...
    for (let d = 0; d < maxLvl; d++) {
      ...
      const dot = this.add.rectangle(-W / 2 + 50 + d * 14, dotY, 10, 6, filled ? 0x44ff88 : 0x334455);
      container.add(dot);
    }
    ...
    const typeTxt = this.add.text(W / 2 - 8, H / 2 - 8, typeLabel, {
      ...
      color: skill.type === 'weapon' ? '#6688ff' : '#66cc66',
    }).setOrigin(1, 1);
    ...
    zone.on('pointerover', () => {
      bg.clear();
      bg.fillStyle(0x1a2255, 0.98); bg.fillRoundedRect(-W / 2, -H / 2, W, H, 6);
      bg.lineStyle(3, 0xffffff);    bg.strokeRoundedRect(-W / 2, -H / 2, W, H, 6);
